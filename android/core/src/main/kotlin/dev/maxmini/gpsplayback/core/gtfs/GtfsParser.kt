 ...
 * then only rows for those trips are kept.
 */
object GtfsParser {
    ...
    fun parse(
        ...
        val stopById = HashMap<String, LatLon>()
            ...
            when (name) {
                ...
                "stops.txt" -> forEachCsvRecord(reader) { get ->
                    ...
                    if (lat != null && lon != null && lat.isFinite() && lon.isFinite()) {
                        stopById[get("stop_id")] = LatLon(lat, lon)
                    }
                }
            }
        }
        ...
        val finalTrips = if (needFallback.isEmpty()) {
            ...
            trips.map { trip ->
                ...
                sts.sortBy { it.first }
                val points = sts.mapNotNull { stopById[it.second] }
        ...
        return GtfsData(routes, finalTrips, shapes, loadedAt = now, feedName = feedName)
    }
